...
Pre-merge safety review for database migration files. Catches the
...
core captures the operation catalogue and the review framework;
each variant supplies the parser and the ORM-specific gotchas.

| Variant | Migration shape |
|---|---|
| `db-migration-review.prisma.prompt.md` | SQL-ish migration files in `prisma/migrations/`, declarative `schema.prisma` deltas |
| `db-migration-review.typeorm.prompt.md` | TypeScript classes implementing `MigrationInterface` (`up()` / `down()`) |
| `db-migration-review.alembic.prompt.md` | Python files in `alembic/versions/`, `op.*` operations, `upgrade()` / `downgrade()` |
| `db-migration-review.ef-core.prompt.md` | C# classes extending `Migration`, `MigrationBuilder` API, `Up()` / `Down()` |
| `core/db-migration-review.core.prompt.md` | Shared scaffold — unsafe-operation catalogue, severity model, output format. Not invoked directly. |
...
2. Rename to `db-migration-review.<orm>.prompt.md`.
...
a populated table it's an outage. The review is context-aware in
that sense.
...
migration reviews are point-in-time artefacts and don't benefit
