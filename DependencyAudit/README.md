...
in one report. Paired with `dependency-fix-*` per-stack action prompts
that bump versions, remove unused deps, and verify between each
category.
...
|---|---|---|
| `dependency-audit.npm.prompt.md` | `package.json` (+ npm / pnpm / yarn lockfile) | `npm outdated`, `npm audit`, `npm dedupe`, `knip` / `depcheck` |
| `dependency-audit.python.prompt.md` | `pyproject.toml` / `requirements*.txt` | `pip list --outdated`, `pip-audit`, `deptry` |
| `dependency-audit.dotnet.prompt.md` | `*.csproj` / `Directory.Packages.props` | `dotnet list package --outdated/--vulnerable/--deprecated` |
| `dependency-audit.swift.prompt.md` | `Package.swift` / `Podfile` | `swift package show-dependencies`, `pod outdated` |
| `dependency-audit.terraform.prompt.md` | `versions.tf` / `required_providers` | `terraform providers`, `tfsec` / `trivy` |
| `core/dependency-audit.core.prompt.md` | Shared scaffold | Not invoked directly. |
...
2. Rename to `dependency-audit.<ecosystem>.prompt.md`.
